...
export async function runDaily(args: DailyArguments, deps: Readonly<{ fetchImpl?: typeof fetch; apiFootballKey?: () => string | undefined; oddsApiKey?: () => string | undefined; now?: () => Date }> = {}): Promise<DailyRunResult> {
  ...
  if (!args.dryRun) { const oddsKey = deps.oddsApiKey?.(); if (oddsKey) { const startedAtUtc=new Date().toISOString(); oddsRequests = 1; try { const client = new TheOddsApiClient({ apiKey: oddsKey, fetchImpl: deps.fetchImpl ?? fetch, clock: nowClock }); const response = await client.upcoming(); const published = await evidenceStore!.publish({ providerKey: "the-odds-api", endpointKey: "odds-upcoming", capturedAtUtc: response.capturedAtUtc, mediaType: "application/json", bytes: response.rawBytes, sourceReference: `daily:${args.sportsDate}:odds` }); if (!published.ok) throw new Error("ODDS_EVIDENCE_FAILED"); evidence.push(published.descriptor); audits.push({ providerKey: "the-odds-api", endpointKey: "odds-upcoming", startedAtUtc, finishedAtUtc: new Date().toISOString(), httpStatus: response.httpStatus, classification: "SUCCESS" }); for (const [id, quotes] of extractQuotes(deep.map((d) => d.fixture), response.events)) quotesByFixture.set(id, quotes); } catch { audits.push({ providerKey: "the-odds-api", endpointKey: "odds-upcoming", startedAtUtc, finishedAtUtc: new Date().toISOString(), httpStatus: null, classification: "UNAVAILABLE" }); } } }
