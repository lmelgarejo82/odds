...
describe("motor diario D+1", () => {
  ...
  it("dry-run es sintético, determinista y hace cero red", async () => {
    ...
    const first=await runDaily(args,{fetchImpl:fetchImpl as typeof fetch,now:()=>new Date("2026-08-03T12:00:00Z")}); const second=await runDaily(args,{fetchImpl:fetchImpl as typeof fetch,now:()=>new Date("2026-08-03T12:00:00Z")}); expect(fetchImpl).not.toHaveBeenCalled(); expect(first).toEqual(second); expect(first.runMode).toBe("MODEL_ONLY_PROVISIONAL"); expect(first.fixturesExcluded).toBe(1); expect(first.apiFootballRequests).toBe(0); expect(first.apiFootballBudget).toBe(3); expect(first.oddsBudget).toBe(3);
  });
  it("adapter The Odds usa sport key y ventana allowlisted, fetch inyectado y redirect manual", async () => {
    const fetchImpl=vi.fn(async (input: string | URL,init?:RequestInit)=>{const url=new URL(input);expect(url.origin).toBe("https://api.the-odds-api.com");expect(url.pathname).toBe("/v4/sports/soccer_epl/odds/");expect(url.searchParams.get("commenceTimeFrom")).toBe("2026-08-04T17:40:00.000Z");expect(url.searchParams.get("commenceTimeTo")).toBe("2026-08-04T18:20:00.000Z");expect(init?.redirect).toBe("manual");return new Response(JSON.stringify([{id:"e1",sport_key:"soccer_epl",commence_time:"2026-08-04T18:00:00Z",home_team:"Olimpia",away_team:"Cerro Porteño",bookmakers:[]}]),{status:200,headers:{"content-type":"application/json"}})});
    const client=new TheOddsApiClient({apiKey:"test-key",fetchImpl:fetchImpl as typeof fetch,clock:{nowUtc:()=>"2026-08-03T12:00:00.000Z"}}); const result=await client.bySport({sportKey:"soccer_epl",commenceTimeFrom:"2026-08-04T17:40:00Z",commenceTimeTo:"2026-08-04T18:20:00Z"}); expect(result.events).toHaveLength(1); expect(fetchImpl).toHaveBeenCalledTimes(1);
  });
  it("adapter The Odds bloquea redirects", async () => { const client=new TheOddsApiClient({apiKey:"test-key",fetchImpl:vi.fn(async()=>new Response(null,{status:302})) as typeof fetch,clock:{nowUtc:()=>"2026-08-03T12:00:00.000Z"}}); await expect(client.bySport({sportKey:"soccer_epl",commenceTimeFrom:"2026-08-04T17:40:00Z",commenceTimeTo:"2026-08-04T18:20:00Z"})).rejects.toThrow("ODDS_REDIRECT_BLOCKED"); });
