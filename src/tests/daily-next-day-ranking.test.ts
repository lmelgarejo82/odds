...
describe("motor diario D+1", () => {
  ...
  it("adapter The Odds usa sport key y ventana allowlisted, fetch inyectado y redirect manual", async () => {
    const fetchImpl=vi.fn(async (input: string | URL,init?:RequestInit)=>{const url=new URL(input);expect(url.origin).toBe("https://api.the-odds-api.com");expect(url.pathname).toBe("/v4/sports/soccer_epl/odds/");expect(url.searchParams.get("commenceTimeFrom")).toBe("2026-08-04T17:40:00Z");expect(url.searchParams.get("commenceTimeTo")).toBe("2026-08-04T18:20:00Z");expect(init?.redirect).toBe("manual");return new Response(JSON.stringify([{id:"e1",sport_key:"soccer_epl",commence_time:"2026-08-04T18:00:00Z",home_team:"Olimpia",away_team:"Cerro Porteño",bookmakers:[]}]),{status:200,headers:{"content-type":"application/json"}})});
