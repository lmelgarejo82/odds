...
export const STATAREA_PROSPECTIVE_ALLOWED_DATE="2026-07-23";
...
export function validateStatareaDate(value:string):string{if(!/^\d{4}-\d{2}-\d{2}$/.test(value))throw new Error("INVALID_DATE_FORMAT");const parsed=new Date(`${value}T00:00:00.000Z`);if(Number.isNaN(parsed.valueOf())||parsed.toISOString().slice(0,10)!==value)throw new Error("INVALID_DATE");const historical=value>=STATAREA_HISTORY_FROM&&value<=STATAREA_HISTORY_TO;if(!historical&&value!==STATAREA_PROSPECTIVE_ALLOWED_DATE)throw new Error("DATE_NOT_AUTHORIZED");return value}
