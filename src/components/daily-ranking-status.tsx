...
export async function DailyRankingStatus() {
  ...
  const oddsMessage = run.usableOddsAvailable ? "Cuotas utilizables disponibles" : oddsResponseReceived ? "Respuesta de cuotas recibida, sin coincidencias utilizables" : "Proveedor de cuotas no disponible; continúa la revisión por modelo";
  ...
  return <>
    ...
    <section className="panel daily-warning"><span className="eyebrow">Disponibilidad operativa</span><h2>{oddsMessage}</h2><p>Matcher {run.matcherVersion ?? "odds-matching/automatic-v1"} · cuotas vinculadas: {run.oddsFixturesMatched} · calibración: {calibration.status} · excluidos de esta vista por fecha local: {wrongLocalDateFixtures}.</p><p className="route-links"><Link href={`/historial?date=${run.sportsDate}`}>Ver historial</Link><Link href="/rendimiento">Ver rendimiento</Link></p></section>
